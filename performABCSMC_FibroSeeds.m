function [part_thetas, part_outputs, part_summaries, part_Ds] = performABCSMC_FibroSeeds(N_parts, f_simulate, f_summaries, target_data, params_mins, params_maxs, scale_param, desired_D, visualise, f_visualise)
...
% Calculate the covariance matrix for the summaries based on the initial
% particles. This allows use of the Mahalanobis distance which will prevent
% domination by largely-varying summaries, and help downweight redundant
% summaries
C_S = cov(part_summaries);

% Calculate discrepancies for each particle based on this distance measure
part_Ds = pdist2(part_summaries, target_summaries, 'Mahalanobis', C_S);
...
while looping
    ...
    switch jumping_type
        case 'normal'
            ...
            % required
            parfor k = 1:N_parts
                [part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), accepted(k)] = MVN_move(1, part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), C_S, Ctheta, theta_mins, theta_maxs, scale_param, target_D, target_summaries, part_simulators{k}, f_summaries); 
            end
            ...
            parfor k = 1:N_parts
                [part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), accepted(k)] = MVN_move(R-1, part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), C_S, Ctheta, theta_mins, theta_maxs, scale_param, target_D, target_summaries, part_simulators{k}, f_summaries);
            end
        ...
        case 'transform'
            ...
            % required
            parfor k = 1:N_parts
                [part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), part_logQs(k), accepted(k)] = GMM_move(1, J, part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), C_S, part_logQs(k), N_theta, theta_mins, theta_maxs, scale_param, target_D, target_summaries, Bmix_fit, part_simulators{k}, f_summaries);
            end
            ...
            % Perform the remaining MCMC steps
            parfor k = 1:N_parts
                [part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), part_logQs(k), accepted(k)] = GMM_move(R-1, J, part_thetas(k,:), part_outputs{k}, part_summaries(k,:), part_Ds(k), C_S, part_logQs(k), N_theta, theta_mins, theta_maxs, scale_param, target_D, target_summaries, Bmix_fit, part_simulators{k}, f_summaries);
            end
...
function [theta, output, summaries, D, moved] = MVN_move(N_moves, theta, output, summaries, D, C_S, Ctheta, theta_mins, theta_maxs, scale_param, target_D, target_summaries, f_simulate, f_summaries)
...
for k = 1:N_moves
    ...
    prop_D = pdist2(prop_summaries, target_summaries, 'Mahalanobis', C_S);
...
function [theta, output, summaries, D, logQ, moved] = GMM_move(N_moves, J, theta, output, summaries, D, C_S, logQ, N_theta, theta_mins, theta_maxs, scale_param, target_D, target_summaries, Bmix_fit, f_simulate, f_summaries)
% This function performs MCMC updates ('move steps' or 'mutations') for a
